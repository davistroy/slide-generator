...
def inspect_presentation(pptx_path, template_name):
    ...
    print(
        f'Slide dimensions: {prs.slide_width.inches:.2f}" x {prs.slide_height.inches:.2f}"'
    )
    ...
    for idx, slide in enumerate(prs.slides, 1):
        ...
        if idx == 1:  # Title slide
            if not any(
                "Block 1 Week 1" in text or "Markdown" in text
                for text in info["text_content"]
            ):
    ...
    slides_with_text = sum(
        1 for slide in prs.slides if inspect_slide(slide, 0)["text_content"]
    )
    slides_with_tables = sum(
        1 for slide in prs.slides if inspect_slide(slide, 0)["has_table"]
    )
    slides_with_images = sum(
        1 for slide in prs.slides if inspect_slide(slide, 0)["has_image"]
    )
