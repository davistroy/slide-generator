...
def inspect_presentation(path, name):
    ...
    for slide_idx, slide in enumerate(prs.slides, start=1):
        ...
        for shape in slide.shapes:
            if hasattr(shape, 'text'):
                ...
                if '*' in t and '**' not in t:
                    ...
                    issues.append(f'Slide {slide_idx}: single asterisk (*) found in text')
        ...
        print(f'  Slide {slide_idx:2d}: {len(text_parts):2d} shapes, {images} img, {tables} tbl, {total_chars:4d} chars | {preview}...')
...
def main():
    ...
    print(f'{"="*80}')
    print(f'Slides:      CFA={cfa_result["slides"]:2d}  Stratfield={stratfield_result["slides"]:2d}')
    print(f'Images:      CFA={cfa_result["stats"]["images"]:2d}  Stratfield={stratfield_result["stats"]["images"]:2d}')
    print(f'Tables:      CFA={cfa_result["stats"]["tables"]:2d}  Stratfield={stratfield_result["stats"]["tables"]:2d}')
    print(f'Characters:  CFA={cfa_result["stats"]["chars"]:5d}  Stratfield={stratfield_result["stats"]["chars"]:5d}')
    print(f'Issues:      CFA={len(cfa_result["issues"]):2d}  Stratfield={len(stratfield_result["issues"]):2d}')
    ...
    if total_issues == 0:
        print('[PASS] Both presentations generated successfully with brand-specific images')
