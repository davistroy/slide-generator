...
LAYOUTS = {
    ...
    "content_text_and_image": {
        "background_color": "#F2F2F2",
        "left_panel": {"x": 0.0, "y": 0.0, "w": 3.41, "h": 5.34, "fill": "#296057"},
        "title": {
            "x": 0.16,
            "y": 0.07,
            "w": 3.16,
...
class StratfieldPresentation(PresentationTemplate):
    ...
    def _add_textbox(self, slide, spec: dict, text: str):
        ...
        shape = slide.shapes.add_textbox(
            Inches(spec["x"]), Inches(spec["y"]), Inches(spec["w"]), Inches(spec["h"])
        )
    ...
    def _apply_bullet_formatting(
        self, paragraph, level: int, text_color: str = "#637878"
    ):
        """
        ...
        insert_index = list(pPr).index(defRPr) if defRPr is not None else len(list(pPr))
    ...
    def _add_footer_elements(self, slide, layout: dict):
        ...
        if "slide_number" in layout:
            ...
            shape = slide.shapes.add_textbox(
                Inches(sn["x"]), Inches(sn["y"]), Inches(sn["w"]), Inches(sn["h"])
            )
            tf = shape.text_frame
            p = tf.paragraphs[0]
    ...
    def add_title_slide(self, title: str, subtitle: str = "", date: str = "") -> None:
        ...
        if bg_path.exists():
            slide.shapes.add_picture(
                str(bg_path), Inches(0), Inches(0), Inches(10), Inches(5.625)
            )
        else:
    ...
    def add_text_and_image_slide(
        self, title: str, bullets: list, image_path: str
    ) -> None:
        """
