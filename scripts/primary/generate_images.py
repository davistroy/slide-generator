...
def load_config(config_path: str = "prompt_config.md") -> dict:
    ...
    if not os.path.exists(config_path):
        ...
        print("[WARNING] Using embedded defaults")
...
def generate_image(
    client, prompt: str, style_config: dict, config: dict, resolution: str = "small"
) -> bytes:
...
"""
    ...
    print(
        f"      Generating with aspect ratio: {aspect_ratio}, image size: {image_size}"
    )
...
Note:
  ...
  reorganized into scripts/primary/. It now supports all resolutions.
    ...
    parser.add_argument(
        "--output", "-o", help="Output directory for images (default: same as input)"
    )
    ...
    print(
        f"    Config loaded from: {config_path if config_exists else 'embedded defaults'}"
    )
    ...
    for i, prompt_file in enumerate(prompt_files, 1):
        ...
        if output_image.exists():
            print(
                f"[{i:2d}/{len(prompt_files)}] Slide {slide_num:2d}: {console_msgs['skip_message']}"
            )
            total_generated += 1
            continue

        print(
            f"[{i:2d}/{len(prompt_files)}] Slide {slide_num:2d}: {console_msgs['generating_message']}"
        )
        ...
        try:
            ...
            image_data = generate_image(
                client, prompt_text, style_config, config, resolution
            )
            ...
            success_msg = console_msgs["success_message"].format(
                filename=output_image.name, size_mb=file_size
            )
