...
def parse_slides_ordered(file_path):
    ...
    header_matches = list(
        re.finditer(header_regex, content, flags=re.IGNORECASE | re.MULTILINE)
    )
    ...
    if not parsed_slides:
        print(
            "Error: No slides found. The script expects headers containing 'Slide N' (e.g., '## **Slide 1**')."
        )
        sys.exit(1)
...
def get_style_instruction(style_path):
    ...
    try:
        ...
        print(
            "Tip: If you have comments in your JSON, remove them. Standard JSON does not support comments."
        )
        sys.exit(1)


def generate_image_for_slide(
    client, style_data, slide_num_str, slide_content, output_dir, args
):
    ...
    if output_file_path.exists() and not args.force:
        print(
            f" > Skipping Slide {slide_num_str}: File exists (use --force to overwrite)"
        )
        return
        ...
        try:
            ...
            if response.candidates:
                ...
                if "SAFETY" in str(finish_reason):
                    print(
                        " > BLOCK: Image blocked by safety filters. Check prompt or style content."
                    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Batch generate PPT slide images using Gemini 3 Pro."
    )
    ...
    parser.add_argument(
        "--fast",
        action="store_true",
        help="Use standard resolution (faster/cheaper) instead of 4K.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Parse slides and print intent without calling API.",
    )
    parser.add_argument(
        "--force", action="store_true", help="Overwrite existing image files."
    )
    parser.add_argument(
        "--notext",
        action="store_true",
        help="Instruct the model NOT to render text in the image.",
    )
    ...
    for i, (slide_num_str, slide_content) in enumerate(slides_list):
        ...
        generate_image_for_slide(
            client, style_text, slide_num_str, slide_content, output_dir, args
        )
        print("-" * 30)
